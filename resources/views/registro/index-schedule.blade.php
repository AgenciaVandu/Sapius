            ...
            var table = $('#dataTable')
                ...
                .DataTable({
                    ...
                    columns: [{
                        ...
                        {
                            data: 'instructor.nombre_completo',
                            orderable: true,
                        },
                        {
                            data: 'category.name',
                    ...
                    "rowCallback": function(row, data) {
                        $(row).find('td:eq(6)').html(
                            '<form method="POST" action="{{ route('descuento.index') }}"> @csrf <input name="cp_id" type="hidden" value="' +
                            data['id'] + '"> <input name="curso_id" type="hidden" value="' + data[
                                'curso_id'] +
                            '"><button type="submit" class="btn btn-primary"><i class="fas fa-barcode"></i></button> </form>'
                        );
                        $(row).find('td:eq(7)').html(
                            '<form method="POST" action="{{ route('contenido.index') }}"> @csrf <input name="cp_id" type="hidden" value="' +
                            data['id'] + '"> <input name="curso_id" type="hidden" value="' + data[
                                'curso_id'] +
                            '"><button type="submit" class="btn btn-primary"><i class="fas fa-book"></i></button> </form>'
                        );
                        $(row).find('td:eq(8)').html(
                            '<form method="POST" action="{{ route('schedule.edit') }}"> @csrf <input name="cp_id" type="hidden" value="' +
                            data['id'] + '"> <input name="curso_id" type="hidden" value="' + data[
                                'curso_id'] +
                            '"><button type="submit" class="btn btn-primary"><i class="fas fa-edit"></i></button> </form>'
                        );
                        $(row).find('td:eq(9)').html(
                            '<form method="POST" action="{{ route('schedule.destroy') }}"> @csrf <input name="cp_id" type="hidden" value="' +
                            data['id'] + '"> <input name="curso_id" type="hidden" value="' + data[
                                'curso_id'] +
